...
/// - Fixed issue in Reinitialise() - y,z and w variables were not being reset.
///   
...
public class FastRandom
{
    ...
    /// </summary>
    private static readonly FastRandom SeedRng = new(Environment.TickCount);
    #endregion
    ...
    //USed for testing to rest the internal syate to know values
    public void ForceValues(uint x, uint y, uint z, uint w)
    {
        _x = x;
        _y = y;
        _z = z;
    ...
    public string Values => $"x:{_x}, y:{_y}, z:{_z}, w:{_w}";
    ...
    public FastRandom()
    {
        Reinitialise(SeedRng.NextIntInclusiveMaxValue());
    }
    ...
    /// </summary>
    public int NextIntInclusiveMaxValue()
    {
        uint t = _x ^ _x << 11;
        _x = _y; _y = _z; _z = _w;
    ...
    public int NextIntInclusiveUpperBound(int lowerBound, int inclusiveUpperBound)
    {
        return Next(lowerBound, inclusiveUpperBound + 1);
    }
    ...
    {
        return (float)(lowerBound + NextDouble() * (upperBound - lowerBound));
    }

    public Vector2 NextVector2(float lowerBoundX, float upperBoundX, float lowerBoundY, float upperBoundY)
    {
        return new Vector2(NextFloat(lowerBoundX, upperBoundX), NextFloat(lowerBoundY, upperBoundY));
    }

    public Vector4 NextVector4(Vector4 lowerBound, Vector4 upperBound)
    {
        return new Vector4(NextFloat(lowerBound.X, upperBound.X), NextFloat(lowerBound.Y, upperBound.Y), NextFloat(lowerBound.Z, upperBound.Z), NextFloat(lowerBound.W, upperBound.W));
    }

    public bool PercentageChance(int chance) => RandomHelper.FastRandom.Next(100) < chance;
    #endregion
}
